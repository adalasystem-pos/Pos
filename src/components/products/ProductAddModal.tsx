...
import { PlusCircle, CheckCircle2, ShieldAlert, DollarSign, Tag, Power } from 'lucide-react';
...
export const ProductAddModal: React.FC<ProductAddModalProps> = ({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!isAdminOrManager) {
      error('تەنها بەڕێوەبەر (Admin/Manager) دەتوانێت ئایتمی نوێ زیاد بکات.');
      return;
    }
    ...
    try {
      ...
      setValidationError(err.message || 'زیادکردنی ئایتم سەرکەوتوو نەبوو');
    } finally {
  ...
  return (
    <Modal
      isOpen={isOpen}
      onClose={onClose}
      title="زیادکردنی ئایتمی نوێ بۆ مینیو"
      ...
      <form onSubmit={handleSubmit} className="space-y-4 text-right">
        {!isAdminOrManager && (
          <div className="p-3 bg-red-50 border border-red-200 rounded-2xl flex items-center gap-2 text-xs text-red-800 font-bold">
            ...
            <span>تەنها بەڕێوەبەر مافی زیادکردنی ئایتمی نوێی هەیە.</span>
          </div>
        )}
        ...
        <div>
          ...
          <input
            ...
            placeholder="نموونە: باڵی مریشکی برژاو بە کونجی"
            required
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3.5">
          ...
          <div>
            ...
            <select
              id="add-product-category-select"
              value={categoryId}
        ...
        <div className="p-3.5 bg-orange-50/60 rounded-2xl border border-orange-100 flex items-center justify-between">
          <div className="space-y-0.5">
            <span className="text-xs font-black text-gray-800 flex items-center gap-1.5">
              ...
              <span>بەردەست بۆ فرۆشتن دەستبەجێ</span>
            </span>
            <p className="text-[11px] text-gray-500 font-medium">
              {active ? 'ئەم خواردنە لە بەشی POS بەردەست دەبێت' : 'لە ئێستادا وەک ناچالاک پاشەکەوت دەبێت'}
            </p>
          </div>

          <label className="relative inline-flex items-center cursor-pointer">
            <input
              id="add-product-active-toggle"
              type="checkbox"
              checked={active}
          ...
          <div className="flex items-center justify-between">
            ...
            <input
              ...
              onChange={(e) => {
                ...
                setAllowPortions(checked);
                if (checked && price > 0) {
                  setHalfPortionPrice(Math.round(price * 0.6));
                  setKiloPrice(Math.round(price * 4));
                }
              }}
              disabled={!isAdminOrManager || isSubmitting}
